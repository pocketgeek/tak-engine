...
void OptionsScreen::build(int channels) {
    ...
    // Supersampling AA as a 4-stop slider: OFF / 2X / 4X / 8X (level 0..3 <-> 0/2/4/8).
    slider("ANTI-ALIASING", 0, 3,
           [&] { return s_.antiAlias == 8 ? 3.0f : s_.antiAlias == 4 ? 2.0f : s_.antiAlias == 2 ? 1.0f : 0.0f; },
           [&](float v) { int l = int(v + 0.5f); s_.antiAlias = l >= 3 ? 8 : l >= 2 ? 4 : l >= 1 ? 2 : 0; },
           [](float v) { int l = int(v + 0.5f); return std::string(l >= 3 ? "8X" : l >= 2 ? "4X" : l >= 1 ? "2X" : "OFF"); });
