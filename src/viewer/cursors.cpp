...
bool CursorSet::load(SDL_Renderer* ren, hpi::Vfs& vfs) {
    if (!ren) return false;
...
void CursorSet::draw(SDL_Renderer* ren, CursorId c, int mouseX, int mouseY, SDL_Color tint) {
    if (!ok_ || !ren) return;
    ...
    SDL_Rect dst{ mouseX - f.hx, mouseY - f.hy, f.w, f.h };   // anchor the hotspot on the pointer
