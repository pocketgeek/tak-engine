...
private:
    ...
    void bakeSprites(const std::string& typeId, int slot, bool canMove, bool canFly) {
        ...
        auto capture = [&](int fi, SDL_Rect& outR, SDL_FRect& outB) {
            ...
            // Retail has no flyer facing branch (root matrix 0x4ee620 is identical
            // for every unit): flyers face -heading exactly like ground movers.
            // Model front is authored at -z for all units, flyers included, so
            // -heading points the front (head/breast) along the movement vector.
    ...
    void buildUnitGeom(const tak::sim::Unit& u, UnitGeom& g, std::vector<Tri>& scratch) {
        ...
        // Flyers face -heading exactly like ground movers (retail has no flyer
        // facing branch; model front is authored at -z for all units).
