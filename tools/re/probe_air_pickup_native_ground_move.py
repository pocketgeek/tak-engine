#!/usr/bin/env python3
"""Join an Araarch native Move route to ZONROC's VTOL pickup.

This composes the shipped-map GROUND2 route/mover setup with retail's VTOL
pickup flight pursuit and boarding. Araarch receives an ordinary retail
Move_Ground order to a native-grade-valid shoreline endpoint on Lake Lokken;
its path is searched using the full shipped TNT GROUND2 grade plane and run by
the native ground mover while ZONROC's native VTOL_Pickup follows it. This is a
diagnostic for whether that order composition boards: currently the carrier
order retires before attachment. The emulator retains controlled boundaries
for path-worker scheduling callbacks, visibility, feature definitions,
effects/UI, and empty COB method tables. No retail GUI is launched.
...
def register_orders(phase, carrier, passenger, carrier_name, passenger_name,
    ...
    for code, target, order, unit in (
            (28, goal, passenger_order, passenger),
        ...
        if error or result != order:
            raise RuntimeError(("native Move_Ground/VTOL_Pickup constructor",
                                code, result, error))
        _, error = icd.call(0x4D7750, (unit, order))
        if error:
            raise RuntimeError(("native move/pickup order insertion", code, error))
    ...
    if get(carrier + 0xC4) or get(passenger + 0xC4) != carrier_order + 0x12:
        raise AssertionError(("ordinary Move_Ground/VTOL_Pickup target references",
...
def run(args):
    ...
    # The full-map route to ZONROC's distant initial cell crosses a GROUND2-
    # blocked water section. This endpoint is on the already validated
    # contiguous Araarch shoreline at z=350; VTOL_Pickup pursues the live unit
    # as it walks east along it.
    goal_cell = (247, TARGET[1])
